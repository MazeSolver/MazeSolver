  ...
  private Neighbour getNeighbour () {
    ...
    while (i < IncludedCells.size() && !next) {
      while (j < IncludedCells.get(i).size() && !next && IncludedCells.get(i).get(j).visited) {
        if (!IncludedCells.get(i - 1).get(j).visited || !IncludedCells.get(i).get(j - 1).visited
            || !IncludedCells.get(i + 1).get(j).visited || !IncludedCells.get(i).get(j + 1).visited) {
          do {
          ...
          while ((random == neighbour.dir.UP.val &&
                  !IncludedCells.get(i).get(j).UP &&
                  !IncludedCells.get(i - 1).get(j).visited) ||
                 (random == neighbour.dir.LEFT.val &&
                  !IncludedCells.get(i).get(j).LEFT &&
                  !IncludedCells.get(i).get(j - 1).visited) ||
                 (random == neighbour.dir.DOWN.val &&
                  !IncludedCells.get(i).get(j).DOWN &&
                  !IncludedCells.get(i + 1).get(j).visited) ||
                 (random == neighbour.dir.RIGHT.val &&
                  !IncludedCells.get(i).get(j).RIGHT &&
                  !IncludedCells.get(i).get(j + 1).visited ));
          next = true;
